...
const { email: VALID_EMAIL, fullName: FULL_NAME, password: VALID_PASSWORD, registerUrl: REGISTER_URL } = registerData;

test.only('Should register successfully using Admin profile', async ({ page }) => {
...
test.only('Should is not register with already Email', async ({ page }) => {
  const registerPage = new RegisterPage();
